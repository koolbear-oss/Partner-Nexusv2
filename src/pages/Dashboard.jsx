...
export default function Dashboard() {
  ...
  const avgPartnerRating = partners.length > 0 
    ? (partners.reduce((sum, p) => sum + (p.avg_project_rating || 0), 0) / partners.filter(p => p.avg_project_rating > 0).length).toFixed(1)
    : 0;
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Partner Network Dashboard</h1>
        <p className="text-slate-600 mt-2">Real-time insights into your partner ecosystem</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="border-l-4 border-l-blue-600 shadow-sm hover:shadow-md transition-shadow">
          ...
          <CardContent>
            ...
            <p className="text-xs text-slate-500 mt-2">
              {pendingOnboarding > 0 && `+${pendingOnboarding} pending onboarding`}
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="border-l-4 border-l-green-600 shadow-sm hover:shadow-md transition-shadow">
          ...
          <CardContent>
            ...
            <p className="text-xs text-slate-500 mt-2">
              {tenderProjects} in tender/matching stage
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="border-l-4 border-l-orange-600 shadow-sm hover:shadow-md transition-shadow">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-slate-600">Avg Partner Rating</CardTitle>
            <Award className="w-5 h-5 text-orange-600" />
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold text-slate-900">{avgPartnerRating}</div>
            <p className="text-xs text-slate-500 mt-2">Out of 5.0 stars</p>
          </CardContent>
        </Card>
      </div>
        ...
        <Card className="border-l-4 border-l-red-500 bg-red-50/50">
          <CardHeader>
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-base text-red-900">Certification Alerts</CardTitle>
                <p className="text-sm text-red-700 mt-1">
                  {expiringCerts} certifications expiring within 30 days - action required
                </p>
              </div>
            </div>
          </CardHeader>
                ...
                <Link key={project.id} to={createPageUrl(`ProjectDetail?id=${project.id}`)}>
                  <div className="p-4 rounded-lg bg-slate-50 hover:bg-slate-100 transition-colors cursor-pointer">
                    ...
                    <div className="flex items-center gap-4 mt-2 text-xs text-slate-500">
                      ...
                      <span>•</span>
                      <span>{project.solution_ids?.length || 0} solutions</span>
                    </div>
                  </div>
                </Link>
              ))}
      ...
      {/* Quick Actions */}
      <Card className="shadow-sm bg-gradient-to-br from-blue-600 to-blue-700 text-white">
        <CardContent className="py-8">
          <h3 className="text-xl font-bold mb-4">Quick Actions</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Link to={createPageUrl('Partners')}>
              <button className="w-full px-4 py-3 bg-white/20 hover:bg-white/30 backdrop-blur rounded-lg transition-colors text-sm font-medium">
                Add Partner
              </button>
            </Link>
